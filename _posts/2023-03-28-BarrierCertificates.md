...
Namely all unsafe states give a negative barrier value, the starting state is safe, and the barrier certificate for the worst-next-state from an already safe state will give a positive value. 
...
The third requirement is satisfied by first finding the aforementioned worst next state that stems from an already known valid state, called $$s*$$. This is calculated with the MALA algorithm, a stochastic gradient Langevin algorithm. Remember that since $$s*$$ is the next state of a valid state, we want our neural network to also certify it as safe. For this worst-case scenario, the barrier function $$\phi$$ is trained to maximize its output. The authors formulated this as the below min-max problem

$$
\underset{\phi}{C*} (h_{\phi}, U, \pi) = \underset{\phi}{min} \quad \underset{s' \in \hat{T}}{max} -h(s')
$$

Where the term $$\underset{\phi}{C*}$$ is derived from $$C_{h}$$ is the set of all valid states.
...
The first step of CRABS is to pre-train a soft-actor-critic policy, $$\pi_{init}$$[^Haarnoja2018] until one is satisfied that the agent behaves safely.
...
Exploration has added new trajectories to our buffer of simulations, $$\hat{D}$$. This allows us to recalibrate our dynamics model, $$\hat{T}$$. $$\hat{T}$$ is parameterized by multiple neural networks, $$\omega$$, and updated by minimizing the negative log-likelihood of the sequences in the replay buffer happening:
